...
class Admin(ctk.CTk):
    def __init__(self):
        ...
        self.img1 = Image.open("pictures\\admin_person_user_man_2839.png")
        self.img1 = self.img1.resize((150, 110))
        ...
        self.lbl_f2 = ctk.CTkLabel(self.frame3,
                                   text="تذکر: این پنل مخصوص آپدیت کردن نام کاربری و پسوورد برای ورود به برنامه است در انتخاب این موارد دقت کنید",
                                   font=("b nazanin", 14),
                                   text_color="white")
        ...
        self.check_box = ctk.CTkCheckBox(self.frame4, text="show Password", command=self.Hide_pass, text_color="white")
    ...
    def Hide_pass(self):
        if self.check_box.get():
            self.check_box.configure(text="hide password")
            self.txt_password.configure(show="")
        else:
            self.check_box.configure(text="show password")
            self.txt_password.configure(show="*")
    ...
    def sub(self):
        ...
        if username != "" and password != "":
            self.my_cursor.execute(
                f"UPDATE admin_user SET username_ = '{username}', password_ = '{password}' WHERE user_id = 1;")
            self.db.commit()
