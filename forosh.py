...
class Forosh(ctk.CTk):
    def __init__(self):
        ...
        self.box_tarafhesab = ctk.CTkComboBox(self.frame1, width=200, font=("b nazanin", 15, "bold"), values=options_tarikh[0])
        ...
        self.search_btn_kala = ctk.CTkButton(self.frame2_1, text="جستجو", width=150, height=35, font=("Arial", 15,"bold"),
        ...
        self.btn3 = ctk.CTkButton(self.frame5, text="ثبت نهایی", width=120, height=30, font=("b nazanin", 15,"bold"),
                                         fg_color="#458205", bg_color="white", border_color="white", border_width=3, command=self.end_sub)
    ...
    def search_kala(self):
        ...
        for item in self.my_cursor:
            ...
            self.btn = ctk.CTkButton(self.inner_frame, text=f"{str(item[1])} - {item[3]} - {item[2]} - {item[4]}", fg_color="#d06ffc"
                                    , text_color="white", border_width=2, border_color="#cf4bfa", height=50 , font=("Arial", 20, "bold")
    ...
    def result(self, x):
        ...
        def Sub():
            ...
            if int(countt) <= int(count):
                self.table1.insert('', "end", text="1", value=[f"{int(price_all_by_mall):,}", f"{int(mal):,}", f"{int(price_all_by_takhfif):,}", f"{int(takhfif):,}", f"{int(price_all):,}", f"{int(pricee):,}", f"{int(countt):,}", Id_, f"{int(radif):,}"])
        ...
        txt_mal = ctk.CTkEntry(win, width=300, height=50, placeholder_text="مالیات: به صورت پیش فرض %6", font=("b nazanin", 20,"bold"))
        txt_mal.pack(pady=5)
    ...
    def delete_select(self):
        ...
        if selection != ():
            ...
            if qustion:
                ...
                count = 1
                for i in tbl:
                    self.table1.insert('', "end", text="1", value=[i[0], i[1], i[2], i[3], i[4], i[5], i[6], i[7], [count]])
                    count += 1
        ...
        def close():
            ...
            for row in self.table1.get_children():
                ...
                for kala in self.my_cursor:
                    ...
                    self.my_cursor.execute(f'INSERT INTO forosh (code_kafsh, color, gender, size, count, price, Date_forosh) VALUES ("{kala[1]}", "{kala[2]}", "{kala[3]}", {kala[4]}, {int(count)}, {kala[6]}, "{Date}");')
                    self.db.commit()
